import numpy as np
import trimesh

R6_NAMES = ["Head", "Torso", "LeftArm", "RightArm", "LeftLeg", "RightLeg"]

def repair_mesh_part(mesh):
    """
    UV-SAFE CLEANUP:
    - Merges vertices to weld seams (preserves UVs).
    - Fills holes using trimesh's lightweight filling.
    - Decimates heavy meshes to stay within Roblox performance limits.
    """
    if mesh.is_empty:
        return mesh
    
    # 1. Weld nearby vertices (Crucial for closing split edges in the original export)
    # merge_tex=True ensures textures don't shift during the weld.
    mesh.merge_vertices(merge_tex=True, merge_norm=True)
    
    # 2. Basic cleanup
    mesh.remove_infinite_values()
    mesh.remove_unreferenced_vertices()
    mesh.fill_holes()

    # 3. Decimation (Only if the part is excessively dense)
    # Roblox typically prefers parts under 5k-10k triangles.
    if len(mesh.faces) > 8000:
        target = int(len(mesh.faces) * 0.7)
        mesh = mesh.simplify_quadratic_decimation(target)

    # 4. Final shading fix
    mesh.fix_normals()
    
    return mesh

def segment_r6_components(mesh, head_height_ratio=0.22, torso_height_ratio=0.45):
    """
    Groups all disconnected mesh components into 6 R6 parts.
    No slicing occurs here; this preserves the original artist's topology.
    """
    # Split into all loose components (eyes, buttons, hair, body layers)
    all_comps = mesh.split(only_watertight=False)
    
    # Filter out tiny geometry artifacts
    components = [c for c in all_comps if c.area > (mesh.area * 0.0005)]
    
    bounds_min, bounds_max = mesh.bounds
    total_height = bounds_max[1] - bounds_min[1]
    total_width = bounds_max[0] - bounds_min[0]
    center_x = (bounds_min[0] + bounds_max[0]) / 2

    # Define the 3D bounding zones
    head_line = bounds_max[1] - (total_height * head_height_ratio)
    leg_line = bounds_min[1] + (total_height * torso_height_ratio)
    arm_zone_width = total_width * 0.18 

    parts = {name: [] for name in R6_NAMES}

    # Identify the 'Anchor' Head piece to help group nearby small items (like eyes)
    head_candidates = [c for c in components if c.bounds[1][1] > head_line]
    head_candidates.sort(key=lambda c: c.area, reverse=True)
    main_head = head_candidates[0] if head_candidates else None
    
    for comp in components:
        c_max = comp.bounds[1]
        c_center = comp.centroid
        dist_x = abs(c_center[0] - center_x)

        # 1. Head Assignment
        is_near_head = False
        if main_head:
            dist = np.linalg.norm(c_center - main_head.centroid)
            if dist < (total_height * 0.12): is_near_head = True

        if (c_max[1] > head_line or is_near_head) and dist_x < arm_zone_width:
            parts["Head"].append(comp)
            continue

        # 2. Arm Assignment (Outer X-axis)
        if dist_x > arm_zone_width:
            if c_center[0] < center_x:
                parts["LeftArm"].append(comp)
            else:
                parts["RightArm"].append(comp)
            continue

        # 3. Leg Assignment (Lower Y-axis)
        if c_center[1] < leg_line:
            if c_center[0] < center_x:
                parts["LeftLeg"].append(comp)
            else:
                parts["RightLeg"].append(comp)
            continue

        # 4. Torso Assignment (Remainder)
        parts["Torso"].append(comp)

    # Combine the groups into 6 meshes
    final_r6 = {}
    for name in R6_NAMES:
        if parts[name]:
            combined = trimesh.util.concatenate(parts[name])
            final_r6[name] = repair_mesh_part(combined)
        else:
            # Safe placeholder to avoid export crashes
            final_r6[name] = trimesh.Trimesh(vertices=[[0,0,0],[0,0.01,0],[0.01,0,0]], faces=[[0,1,2]])
            
    return final_r6
