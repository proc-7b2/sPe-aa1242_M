...
def repair_mesh_part(mesh):
    """
    UV-SAFE REPAIR: 
    Stitches edges and fills holes without regenerating the mesh structure.
    This preserves original textures and facial details.
    """
    if mesh.is_empty:
        return mesh
    ...
    # 2. Stitch the mesh (UV-Safe)
    # This merges vertices that are in the same physical space but separated
    # merge_tex=True ensures the UV coordinates are kept during the merge
    mesh.merge_vertices(merge_tex=True, merge_norm=True)
    
    # 3. Fill remaining small gaps
    # This adds new faces to actual holes. Note: New faces will have 
    # basic UVs, but original geometry remains untouched.
    mesh.fill_holes()
